...
class _ComprarMarketplacePageState extends State<ComprarMarketplacePage> {
  ...
  void initState() {
    ...
    _loadActiveOrder();
    AnalyticsService.instance.track(AnalyticsEvent.marketplaceOpen);
  }
  ...
  Future<void> _selectCategory(int id) async {
    ...
    if (mounted && _resultsError == null) {
      AnalyticsService.instance.track(AnalyticsEvent.categoryOpen);
    }
  }
  ...
  Future<void> _openProduct(MarketplaceProduct product) async {
    ...
    AnalyticsService.instance.track(AnalyticsEvent.productOpen);
  ...
  Future<void> _requestAccountDeletion() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Tem certeza de que deseja excluir sua conta? Esta acao pode remover permanentemente os dados associados a ela.',
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            child: const Text('Confirmar exclusao'),
          ),
        ],
      ),
    );
    ...
    try {
      ...
      if (!mounted) return;
      _message('Solicitacao de exclusao recebida.');
    } on ApiV1Exception catch (error) {
      if (!mounted) return;
      ...
      if (mounted) {
        _message('Nao foi possivel enviar a solicitacao de exclusao.');
      }
    } finally {
        ...
        actions: [
          if (_accountDeletionLoading)
            ...
            PopupMenuButton<String>(
              onSelected: (value) {
                if (value == 'delete_account') _requestAccountDeletion();
              },
