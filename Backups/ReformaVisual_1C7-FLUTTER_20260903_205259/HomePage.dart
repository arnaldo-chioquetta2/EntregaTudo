...
class _HomePageState extends State<HomePage> {
    ...
    try {
      if (online) {
        ...
        _locationService.startTracking((position) async {
          ...
          await _operationalController.sendMotoboyHeartbeat(
    ...
    try {
      ...
      if (usuarioEhMotoboy && !usuarioEhFornecedor) {
        ...
        if (!hbPausadoPorVenda && !_motoboyTrackingActive) {
          await _operationalController.sendMotoboyHeartbeat(latitude, longitude);
        } else {
      ...
      if (!usuarioEhMotoboy && usuarioEhFornecedor) {
        ...
        if (!hbPausadoPorEntrega) {
          await _operationalController.sendFornecedorHeartbeat(latitude, longitude);
        } else {
  ...
  Future<void> _processaFornecedorComLatLong() async {
    await _operationalController.sendFornecedorHeartbeatFromCurrentPosition();
  }

  Future<void> _processaMotoboyComLatLong() async {
    await _operationalController.sendMotoboyHeartbeatFromCurrentPosition();
  }
