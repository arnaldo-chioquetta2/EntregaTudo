...
void main() {
  ...
  test('aceitar oferta usa API injetada e cria entrega ativa', () async {
    SharedPreferences.setMockInitialValues({'idUser': 4});
    var calls = 0;
    final controller = HomeOperationalController(
      respondToDelivery: (userId, deliveryId, accept) async {
        calls++;
        expect(userId, 4);
        expect(deliveryId, 9);
    ...
    final result = await controller.respondToCurrentOffer(true);
    expect(result.success, isTrue);
    expect(calls, 1);
    expect(controller.entregaAtiva?.idPedido, 9);
    expect(controller.deliveryDataMotoboy, isNull);
    expect(controller.heartbeatPausedByDelivery, isTrue);
    controller.disposeSession();
  });

  test('recusar oferta limpa o estado de entrega', () async {
    SharedPreferences.setMockInitialValues({'idUser': 4});
    final controller = HomeOperationalController(
      respondToDelivery: (_, __, ___) async => true,
    );
    controller.deliveryDataMotoboy = {'chamado': 9};
    controller.heartbeatPausedByDelivery = true;
    final result = await controller.respondToCurrentOffer(false);
    expect(result.success, isTrue);
    expect(controller.deliveryDataMotoboy, isNull);
    expect(controller.heartbeatPausedByDelivery, isFalse);
    controller.disposeSession();
  });

  test('coleta e conclusao usam APIs injetadas e preservam estados', () async {
    SharedPreferences.setMockInitialValues({'idUser': 4});
    var pickupCalls = 0;
    var completionCalls = 0;
    final controller = HomeOperationalController(
      notifyPickedUp: ({idPedido, idMotoboy, codigo}) async {
        pickupCalls++;
        expect(idPedido, 10);
        expect(idMotoboy, 4);
        expect(codigo, '111');
      ...
      notifyDeliveryCompleted: ({idPedido, idMotoboy, codigo}) async {
        completionCalls++;
        expect(idPedido, 10);
        expect(idMotoboy, 4);
        expect(codigo, '123');
    ...
    controller.heartbeatPausedByDelivery = true;
    final picked = await controller.markDeliveryPickedUp();
    final completed = await controller.completeActiveDelivery('123');
    expect(picked.success, isTrue);
    expect(completed.success, isTrue);
    expect(pickupCalls, 1);
    expect(completionCalls, 1);
    expect(controller.entregaAtiva, isNull);
    expect(controller.heartbeatPausedByDelivery, isFalse);
    controller.disposeSession();
  });

  test('codigo invalido nao chama conclusao', () async {
    SharedPreferences.setMockInitialValues({'idUser': 4});
    var calls = 0;
    final controller = HomeOperationalController(
      notifyDeliveryCompleted: ({idPedido, idMotoboy, codigo}) async {
    ...
    final result = await controller.completeActiveDelivery('12');
    expect(result.success, isFalse);
    expect(calls, 0);
    expect(controller.entregaAtiva, isNotNull);
    controller.disposeSession();
  });
}
